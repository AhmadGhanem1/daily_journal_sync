...
class JournalWriter:
    ...
    def _setup_logging(self):
        ...
        self._logger = logger
        self._logger.info(
            "Writer initialized. repo=%s with_weather=%s", self.repo, self.with_weather
        )
        ...
        if not md_path.exists() or md_path.stat().st_size == 0:
            ...
            if self.with_weather:
                try:
                    ...
                    header_lines.append("Weather: N/A")
    ...
    def _maybe_trigger_push(self, md_path: Path):
        """Check file size and trigger git push if >= 10 KB."""
        try:
        ...
        if size >= config.MAX_MD_SIZE_BYTES:
            self._logger.info("File size %d ≥ threshold — triggering git push", size)
            ...
            try:
                subprocess.Popen(
                    ["/bin/bash", str(push_script)],
                    cwd=self.repo,
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                )
                self._logger.info("push.sh launched successfully")
            except Exception as e:
                self._logger.error("Failed to start push.sh: %s", e)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                self._logger.info("Queue interrupted — shutting down.")
                break
